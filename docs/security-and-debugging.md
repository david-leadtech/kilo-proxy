...
Protocols are preserved, subject to the [Anthropic tool-schema bridge](clients.md#fable--anthropic-tool-schemas). Availability depends on Kilo and the selected model; the proxy does not translate between Chat Completions, Responses, and Messages. JSON, tool calls, and SSE otherwise pass through. Upstream errors pass through except for the recognized payload rejection described below. Cancellation propagates upstream. Uploads are limited to 32 MiB; reading a request and waiting for initial upstream headers each have a two-minute limit. Ordinary streaming responses have no write timeout. Requests that publish temporary image URLs have a separate ten-minute total deadline so inference stays within the image lifetime.

Upstream payload limits can be lower than the local upload limit. When an upstream **413** has a complete body of at most 16 KiB and the recognized `FUNCTION_PAYLOAD_TOO_LARGE` code in its `X-Vercel-Error` header or uncompressed body, the proxy returns a clearer JSON diagnostic with guidance to compact the client conversation, start a new one, or reduce attachments. Other 413 responses pass through unchanged. The diagnostic uses `error.code = "upstream_payload_too_large"` and includes `request_bytes` when the outbound length is known. The original rejection remains in the **Gateway response** trace; the **Client response** trace shows the diagnostic. [Vercel Functions document a 4.5 MB body limit](https://vercel.com/docs/functions/limitations#request-body-size). The proxy does not silently remove conversation context or retry the same rejected request. Generated-image MCP previews are bounded from v0.23.1, while original files retain full resolution; existing client histories may still contain large base64 images. See [image payload recovery](codex-images.md#payload-limits-and-413-errors).
...
**Settings → Large images** saves `imageTransport.mode` (`off`, `compress`, `upload`, `cloudflare`, `litterbox`, or `tailscale`), `imageTransport.profile` (`high`, `balanced`, or `small`) and `imageTransport.litterboxTTL` (`1h`, `12h`, `24h`, or `72h`) in `settings.json`. Existing installations and fresh profiles default to **Off**, with **High quality** as the initial compression profile and one hour as the Litterbox expiry. Choices take effect for new Responses, Chat Completions and Anthropic Messages requests without restarting; smaller requests are unchanged. A failure never switches automatically to a different backend.
...
**Cloudflare quick tunnel** and **Tailscale Funnel** publish a separate loopback image server. The public listener serves only registered images at unguessable URLs; it does not serve the administration panel, inference endpoints or arbitrary local paths. Each request's image URLs are removed after completion, failure, or cancellation. The image server and owned foreground tunnel process can be reused until application shutdown; switching modes does not stop an already started tunnel. Kilo Proxy does not reuse the Cursor tunnel or reset an existing Tailscale Serve/Funnel configuration. Tailscale uses HTTPS port 8443 and refuses an existing conflicting route. The respective tunnel executable must already be installed, and Tailscale needs an authorized account and Funnel configuration.

**Litterbox** uploads images anonymously to third-party storage. Its selected expiry is a request to that service, not a deletion guarantee verified by Kilo Proxy. Its API has no early-delete operation: switching modes, canceling a request, or quitting cannot remove an uploaded copy early. The service states that it stores the uploader's IP address and requires approval for commercial service use. See its [official FAQ](https://litterbox.catbox.moe/faq.php).

URL modes preserve image bytes and do not enable request capture. Captured response bodies for URL-backed requests are omitted to avoid retaining temporary links echoed across streaming chunks; metadata and redacted upstream requests remain available. Anyone who obtains an active image URL can retrieve that image, without a Kilo API key.

For Kilo uploads, deletion is requested after completion, failure, or cancellation, with bounded retries and an on-screen warning when cleanup cannot be confirmed. Normal quit waits for bounded cleanup. Link expiry is not deletion. Bookkeeping is in memory only, so a crash or machine shutdown can leave Kilo copies that the next launch cannot clean up automatically. For tunnels, ending the local image server removes local access but cannot retract copies already fetched by another party. See [setup, image limits, and backend-specific lifetime](image-uploads.md).
