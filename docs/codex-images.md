...
**Settings → Large images** offers two optional alternatives for oversized Responses requests. **Compress locally** uses a fixed **High quality**, **Balanced**, or **Small size** profile on outbound copies, leaving originals on disk untouched. **Upload to Kilo · Experimental** preserves image bytes through temporary Kilo links without a tunnel or personal storage setup. The default is **Off**, and compression never switches to uploads automatically. Uploads use Cloud Agent attachment storage outside a documented Gateway integration; cleanup can fail or be interrupted by a crash. Read [profiles, limits, and deletion behavior](image-uploads.md) before choosing a mode.
