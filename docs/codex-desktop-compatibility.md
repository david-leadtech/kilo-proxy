...
Completed proxy requests can be inspected at four stages: original client request, adapted gateway request, original gateway response, and adapted client response. Capture is off by default, including when upgrading older settings without an explicit capture preference. The user can enable it in Activity; the preference survives restarts. When enabled, capture stores at most 30 entries in memory, 128 KiB per body and 32 KiB per header set. Disabling it clears all captured content and request rows immediately, including in-flight copies, while cost and token accounting continues. At most 16 simultaneous requests are captured. Clear history also prevents earlier in-flight requests from restoring cleared entries. No trace is written to disk.
...
## Empty subagent input conflict (0.30.0)

Codex V1's `multi_agent_v1.spawn_agent` and `multi_agent_v1.send_input` declare `message` and `items` as independent optional properties, but their runtime rejects any call containing both. Some models generate a valid message alongside `items: []`, causing `Provide either message or items, but not both` before the subagent can start. The mismatch is visible in Codex's [tool declarations](https://github.com/openai/codex/blob/main/codex-rs/core/src/tools/handlers/multi_agents_spec.rs) and [input parser](https://github.com/openai/codex/blob/main/codex-rs/core/src/tools/handlers/multi_agents_common.rs), and reported in [openai/codex#37037](https://github.com/openai/codex/issues/37037).

Kilo Proxy adds an omission hint to these declared tools and removes only an empty `items` array when a returned call already contains a non-blank string `message`. The message, model, reasoning and fork settings are preserved. Calls containing two substantive inputs, malformed arguments, unrelated tools and V2 tools are left unchanged; the proxy does not choose between competing instructions. Existing conversation history is not rewritten by this correction.

The correction applies to `/v1/responses` across model providers, including namespaced and qualified tool names. JSON responses and streaming argument fragments, completed arguments, output items and final responses use the same adaptation. For the affected calls, argument deltas are withheld until the complete arguments arrive; unrelated calls and response events continue streaming. Anthropic's schema envelope is removed before this correction. Run the updated proxy to enable it; no profile change is required.
