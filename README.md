...
**Settings → Large images** offers **Off** (default), **Compress locally**, or **Upload to Kilo · Experimental** for oversized Responses requests. Local compression uses your choice of **High quality**, **Balanced**, or **Small size**, preserving local originals and never lowering the profile automatically. Experimental uploads preserve the image bytes through temporary Kilo links, without a tunnel or personal storage setup; this is not a documented Gateway integration. Read [compression profiles, upload limits, and deletion behavior](docs/image-uploads.md).
